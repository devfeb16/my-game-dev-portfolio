...
export default function DashboardOverview() {
  ...
  const visibleStats = stats.filter(
    (stat) => user?.role && stat.roles.includes(user.role)
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md-cols-2 lg-cols-3 gap-4">
        {visibleStats.map((stat, index) => (
          <div
            key={index}
            className="bg-[#1a1b2e] border border-[#2a2b3e] rounded-xl p-6 hover-purple-500/50 transition"
          >
      ...
      <div className="bg-[#1a1b2e] border border-[#2a2b3e] rounded-xl p-6">
        ...
        <div className="space-y-3">
          {(user?.role === UserRole.ADMIN || user?.role === UserRole.BLOGGER) && (
            <button className="w-full text-left px-4 py-3 bg-[#0a0b0f] rounded-lg hover-[#2a2b3e] transition text-white">
