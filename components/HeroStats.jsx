...
export default function HeroStats({ stats = defaultStats, className }) {
  return (
    <div className={`grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 ${className || ""}`}>
      {stats.map((s) => (
        <div key={s.label} className="rounded-lg border border-[var(--color-border)]/40 bg-[#0c1117] p-4 text-center">
          ...
          <div className="mt-1 px-1 text-[11px] sm:text-xs md:text-sm lg:text-normal sm:tracking-wide lg:tracking-wider text-zinc-400 leading-tight break-words hyphens-auto whitespace-normal">
            {s.label}
          </div>
        </div>
      ))}
