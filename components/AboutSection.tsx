...
export default function AboutSection() {
  const highlights = [
    { icon: FaGamepad, text: "Mobile Performance", color: "text-neon-cyan" },
    { icon: FaCode, text: "Gameplay Systems", color: "text-neon-blue" },
    ...
    <section className="mx-auto max-w-6xl px-6">
      <div className="grid grid-cols-1 md:grid-cols-[240px_1fr] gap-8 items-start">
        <div className="relative group">
          <div className="relative h-56 w-56 overflow-hidden rounded-xl border-2 border-neon-cyan/30 bg-[#0c0e14]/60 transition-all duration-300 group-hover:border-neon-cyan/60 group-hover:shadow-[0_0_30px_rgba(34,211,238,0.3)]">
            <Image src="/images/avatar.svg" alt="Avatar" fill className="object-cover transition-transform duration-300 group-hover:scale-105" />
            <div className="absolute inset-0 bg-gradient-to-t from-neon-cyan/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
          </div>
          ...
          <div className="mt-6 space-y-4 text-zinc-300">
            ...
            <div className="grid grid-cols-2 gap-3 mt-6">
              {highlights.map((item, idx) => {
                ...
                return (
                  <div key={idx} className="flex items-center gap-2 p-3 rounded-lg bg-[#0c0e14]/40 border border-[var(--color-border)]/20 hover:border-[var(--color-border)]/40 transition-all duration-200">
